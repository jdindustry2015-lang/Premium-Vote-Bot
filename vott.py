...
async def force_sub_prompt(query, giveaway, missing):
    rows = []
    for ch in missing:
    ...
    rows.append([premium_button("✅ VERIFY", callback_data=f"verify_{giveaway.get('id', '')}", emoji_id=EMOJI_CONFIRM, style=BUTTON_STYLE_SUCCESS)])
    await query.edit_message_text(
        "🔒 <b>FORCE SUBSCRIBE REQUIRED</b>\n\n"
        "Join all required channels, then press <b>VERIFY</b>.\n\n"
        "Only verified channel members can vote.",
...
async def handle_vote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # User must be a member of the giveaway channel.
    if not await is_member_of_chat(context.bot, giveaway['channel_id'], voter_id):
        await query.answer("⚠️ Join the giveaway channel first.", show_alert=True)
        return

    # Force-sub verification (0, 1 or 2 channels).
    ...
    if not verified:
        ...
        await query.answer("🔒 Join the required channel(s) first.", show_alert=True)
        try:
    ...
    # One voter can have only one active vote in this giveaway.
    voter_votes = giveaway.setdefault("voter_votes", {})
    ...
    if existing_target is not None:
        if int(existing_target) == target_user_id:
            ...
            await query.answer(f"⚠️ You already voted for {name}.", show_alert=True)
        return
    ...
    save_user(voter_id, query.from_user.username, query.from_user.first_name, query.from_user.last_name, source="vote")
...
async def verify_force_sub_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if giveaway_id not in giveaways:
        ...
        return
    giveaway = giveaways[giveaway_id]
    verified, missing = await verify_force_sub(context.bot, giveaway, query.from_user.id)
    if verified:
        await query.answer("✅ Verified! You can vote now.", show_alert=True)
        try:
            await query.edit_message_text(
                "✅ <b>VERIFIED</b>\n\nYou are subscribed to all required channels.\nGo back to the giveaway and vote.",
                parse_mode="HTML"
            )
        except Exception:
            pass
